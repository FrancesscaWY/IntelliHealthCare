...
- 如果你是前端开发同学，想知道“具体怎么联调、怎么拿 token、curl/Swagger 怎么用、页面和接口怎么对应”，优先看 `docs/frontend-api-integration-guide.md`
- 本文档更适合做“业务范围和接口全量索引”，不作为第一次上手联调的操作手册
...
- 用户端是统一用户侧入口，老人和家属共用同一套前端与 API 体系，仅在权限与展示层做角色差异，不额外拆分多套端实现
...
| `ai/health/summary` | AI 健康摘要 | 汇总当前用户或授权对象的近期健康数据、趋势与注意事项 |
...
| Method | Path | 描述 |
| --- | --- | --- |
| GET | `/ai/health-summary` | 生成当前用户或授权对象的健康摘要 |
