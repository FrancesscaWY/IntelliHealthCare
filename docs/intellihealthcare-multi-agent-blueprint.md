...
用户端的“豆沙包”首先必须表现为一个自然对话的康养助手，而不是领域 Agent 输出的拼接壳层。

该原则带来四个直接约束：

- 默认路径应是 `AssistantConversationAgent -> LLM / Knowledge` 的直接回答链路
- 只有在用户明确涉及个人报告、个人指标、服务决策、预约推进等场景时，才升级到 Specialist 工作流
- 泛知识问答、闲聊、安慰、陪伴、说明类问题，不应因为页面上下文存在就自动触发多智能体编排
- 若运行时未真实接入 LLM，多智能体框架不能替代自然对话能力，必须把模型接入状态视为上线前置条件
...
- 承接统一康养助手会话
- 默认基于 LLM 与知识库直接回答泛问题、陪伴式对话和基础说明
- 仅在明确需要个人化健康背景、报告解读、服务推荐或预约推进时，才交由控制层编排
- 把健康摘要、报告解读、服务推荐和风险提醒整合成自然语言回复，而不是直接回显模块结构
