...
import { currentAdminAuthSession, hasAdminAuthSession } from "@/shared/auth/session";
import {
  LOGIN_PAGE_ID,
...
export function usePageNavigation(options: UsePageNavigationOptions) {
  ...
  const resolveAccessiblePageId = (pageId: string, rememberRedirect = false) => {
    ...
    if (!requiresAdminAuth(normalizedPageId) || hasAdminAuthSession()) {
  ...
  watch(
    ...
    (session) => {
      ...
      if (!requiresAdminAuth(currentPageId)) {
        return;
      }
