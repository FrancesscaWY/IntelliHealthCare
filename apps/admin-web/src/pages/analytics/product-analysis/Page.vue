...
const columns = computed(() => pageData.value.columns as readonly Column[]);
const rows = computed(() => pageData.value.rows as readonly Row[]);
...
const totalBrowse = computed(() => rows.value.reduce((sum, row) => sum + parseNumber(row.browse), 0));
const totalPayUsers = computed(() => rows.value.reduce((sum, row) => sum + parseNumber(row.payUsers), 0));
const totalPayOrders = computed(() => rows.value.reduce((sum, row) => sum + parseNumber(row.payOrders), 0));
const totalAmount = computed(() => rows.value.reduce((sum, row) => sum + parseNumber(row.amount), 0));
const averageConversion = computed(() => {
  if (!rows.value.length) {
    return 0;
  }

  const total = rows.value.reduce((sum, row) => sum + parseNumber(row.conversion), 0);
  return total / rows.value.length;
});

const summaryCards = computed(() => {
  const productCount = rows.value.length;
...
function isImageCell(value: unknown): value is ImageCell {
  ...
  <section class="trade-overview-page product-analysis-page">
    ...
    <article class="panel filter-panel">
      ...
      <section class="dashboard-filter">
        <div class="filter-stack">
          <div v-for="(row, rowIndex) in pageData.filters" :key="rowIndex" class="filter-row-grid">
    ...
    <section class="panel table-panel">
      <header class="panel-head panel-head--between">
        ...
        <button v-if="pageData.bulkActionLabel" class="ghost-button ghost-button--compact" type="button" @click="trigger(pageData.bulkActionLabel)">
          {{ pageData.bulkActionLabel }}
        </button>
      </header>

      <div class="data-table">
        <div class="data-table__scroll">
          <table :style="pageData.tableMinWidth ? { minWidth: `${pageData.tableMinWidth}px` } : undefined">
            <colgroup>
              <col
