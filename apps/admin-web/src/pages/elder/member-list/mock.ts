...
const remoteMembersStorageKey = "admin:elder:api-members";
...
export function readRemoteMembers() {
  if (typeof window === "undefined") {
    return [] as MemberItem[];
  }

  const rawValue = window.sessionStorage.getItem(remoteMembersStorageKey);
  ...
  try {
    ...
    window.sessionStorage.removeItem(remoteMembersStorageKey);
    return [] as MemberItem[];
  }
}

export function saveRemoteMembers(members: MemberItem[]) {
  if (typeof window === "undefined") {
    return;
  }

  window.sessionStorage.setItem(remoteMembersStorageKey, JSON.stringify(members));
}

export function getMemberSource() {
  const remoteMembers = readRemoteMembers();
  return remoteMembers.length > 0 ? remoteMembers : fallbackMembers;
}
