...
import { loadStressSource } from "../measurement-source";
...
const pageData = ref<Awaited<ReturnType<typeof loadStressSource>>>({
  list: [],
  dailyTimeline: [],
  monthlyData: []
});

const healthList = computed<StressRecord[] | null>(() => {
  if (pageData.value.list.length > 0) {
    return pageData.value.list as StressRecord[];
  }
  return null;
});

const stressData = computed(() => healthList.value ?? []);
const timelineEntries = computed<TimelineEntry[]>(() => pageData.value.dailyTimeline as TimelineEntry[]);

const latest = computed(
  () =>
    stressData.value[stressData.value.length - 1] ?? {
      date: "",
      stress: 0,
      time: "",
    }
...
const previous = computed(() => stressData.value[stressData.value.length - 2] ?? latest.value);
...
const averageStress = computed(() => {
  if (!stressData.value.length) return 0;
  const total = stressData.value.reduce((sum, item) => sum + item.stress, 0);
  return Math.round(total / stressData.value.length);
});

const latestChange = computed(() => latest.value.stress - previous.value.stress);
...
function getChartBounds(values: number[]) {
  ...
  const max = Math.min(100, Math.ceil((Math.max(...values) + 2) / 5) * 5);
...
function createDayChart(items: TimelineItem[], width = 320, height = 230) {
  ...
  return {
    ...
    yLabels: [0, 1, 2, 3, 4, 5].map((index) => Math.round(max - ((max - min) / 5) * index)),
...
function formatChange(index: number) {
  if (index === 0) return "—";
  const diff = stressData.value[index].stress - stressData.value[index - 1].stress;
  if (diff === 0) return "持平";
  return `${diff > 0 ? "+" : ""}${diff}`;
}
...
function goToAddData() {
  sessionStorage.setItem("addMetric", "stress");
  sessionStorage.setItem("addReturnPath", "health/data-pressure");
  props.navigation?.navigateTo?.("health/add-data");
}

async function loadPageData() {
  try {
    pageData.value = await loadStressSource();
  } finally {
...
<template>
  <section class="health-data-page">
    <header class="medication-nav">
      ...
      <h1>{{ props.pageEntry?.title || "压力" }}</h1>
    </header>
    ...
    <main class="medication-scroll">
      <template v-if="healthList">
        <section class="latest-small-card">
          <div class="latest-card-main">
            <div>
              ...
              <strong>{{ formatValue(latest.stress) }}</strong>
              <p class="small-card-date">{{ latest.date }}</p>
            </div>
            <div class="latest-card-summary">
              <span>平均压力</span>
              <strong>{{ formatValue(averageStress) }}</strong>
        ...
        <section class="pressure-panel-card">
            ...
            <template v-else-if="selectedChartMode === 'week'">
              <svg class="main-chart" :viewBox="'0 0 ' + weekChart.width + ' ' + weekChart.height" preserveAspectRatio="none">
                ...
                <g v-for="(point, index) in weekChart.points" :key="`week-point-${index}`">
                  <line :x1="point.x" :y1="point.maxY" :x2="point.x" :y2="point.minY" class="stress-range-line" />
                  <circle :cx="point.x" :cy="point.avgY" r="4.5" class="stress-point" />
                </g>
              </svg>
              <div class="chart-axis-left">
          ...
          <div class="summary-strip">
            ...
            <article class="summary-block">
              <span>平均压力</span>
              <strong>{{ formatValue(summary.avg) }}</strong>
            </article>
          </div>
        </section>
        ...
        <section class="detail-table-card">
          ...
          <div class="table-wrapper">
            <table class="metric-table">
              ...
              <tbody>
                <tr v-for="(item, index) in stressData" :key="item.date">
                  <td>{{ item.date }}</td>
                  <td>{{ formatValue(item.stress) }}</td>
      ...
      <footer class="add-area">
        <button class="add-btn" type="button" @click="goToAddData">+ 添加压力记录</button>
      </footer>
    </main>
  </section>
</template>
